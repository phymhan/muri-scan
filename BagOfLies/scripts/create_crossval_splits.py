...
if __name__ == '__main__':
	...
	# ### visualize the duaration of the videos
	# durations = [df.end[i]-df.start[i] for i in range(df.shape[0])]
	# print(min(durations), max(durations))

	# _ = plt.hist(durations, bins='auto') 		# arguments are passed to np.histogram
