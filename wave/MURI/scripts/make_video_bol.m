vidroot = '/media/ligong/Picasso/Datasets/BagOfLies/openface/';
outroot = '/media/ligong/Picasso/Datasets/BagOfLies/attn_vis';
pltroot = '/media/ligong/Picasso/Datasets/BagOfLies/attn_plt';

file_list = dir(fullfile(vidroot, 'User*'));
fl = {};
for i = 1:length(file_list)
    user = file_list(i).name;
    runs = dir(fullfile(vidroot, user, 'run*'));
    for j = 1:length(runs)
        run = runs(j).name;
        filename = sprintf('%s/%s', user, run);
        fl{end+1} = filename;
    end
end
