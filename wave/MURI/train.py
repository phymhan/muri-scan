...
if __name__ == '__main__':
    ...
    for i in range(folds_num):
        ...
        train_dataloader = DataLoader(utils.get_dataset(opt, x_train), shuffle=True, num_workers=0, batch_size=bs)
        val_dataloader   = DataLoader(utils.get_dataset(opt, x_val),   shuffle=False, num_workers=0, batch_size=1)
        
        W = None
        if opt.weighted_CE:
            stats = utils.get_stats(x_train, train_dataloader.dataset.labels)
            total = stats[0] + stats[1]
            W     = torch.Tensor([total/stats[0], total/stats[1]]).cuda()
