...
def viz_durations(path):
	...
	# plt.show()
	plt.savefig("../data/openface/duration.png")
...
def viz_waveforms(path):
	...
	for filename in os.listdir(path):
		...
		print(filename, labels[filename], features.shape)
		fig = plt.figure()
		for i in range(9):
			plt.subplot(3,3,i+1)
		...
		fig.savefig(os.path.join(save_path, directory[labels[filename]], filename+"-1.png"))
		plt.clf()
		fig = plt.figure()
		for i in range(9,17):
			plt.subplot(2,4,i-8)
		...
		fig.savefig(os.path.join(save_path, directory[labels[filename]], filename+"-2.png"), dpi=500)
		plt.clf()
