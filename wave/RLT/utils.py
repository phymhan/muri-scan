...
def get_dataset(opt, file_list):
    ...
    if opt.setting == 'openface':    
        dataset = RLT(opt.dataroot, file_list, opt.labels, opt.num_frames, opt.time_stride)
    else:
...
def get_image_dataset(opt, file_list):
    ...
    if opt.setting == 'openface':    
        dataset = RLTImage(opt.imageroot, file_list, opt.labels, opt.num_frames, opt.time_stride)
    else:
        raise NotImplementedError('Setting [%s] is not implemented.' % opt.setting)
    return dataset
