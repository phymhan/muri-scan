...
def cv_initialize(self):
    '''
    To Handle the custom network initialization
    i.e xavier, kaiming and orthogonal
    Might be helpful for GRUs
    '''
...
def get_model(opt):
    ...
    if len(opt.ce_weight):
        ce_weight = torch.Tensor(opt.ce_weight)
    ...
    lr_scheduler = LRScheduler(policy='StepLR', step_size=opt.lr_step_size, gamma=opt.decay_factor)

    if opt.setting == 'clip':
        if opt.which_model == 'base':
            net = NeuralNetClassifier(
                module=BaseClipModel,
                module__num_classes=opt.num_classes,
                module__use_gru=opt.use_gru,
                module__feature_dim=opt.feature_dim,
                module__embedding_dim=opt.embedding_dim,
                module__gru_hidden_dim=opt.gru_hidden_dim,
                module__gru_out_dim=opt.gru_out_dim,
                module__dropout=opt.dropout,
                module__noisy=opt.noisy,
                criterion,
                optimizer=optim.Adam,
                optimizer__lr=opt.lr,
        ...
        if opt.which_model == 'base':
            net = NeuralNetClassifier(
                ...
                module__dropout=opt.dropout,
                module__noisy=opt.noisy,
                module__dim_input_map=opt.dim_input_map,
                module__norm_input_map=norm_input_map,
                module__dim_fc=opt.dim_fc,
                module__norm_fc=norm_fc,
                criterion,
                optimizer=optim.Adam,
                optimizer__lr=opt.lr,
                callbacks=[lrscheduler],
    ...
    # Custom Initialize Module (Hope it works)
    # NeuralNetClassifier.initialize_module() = cv_initialize()

    ''' Add the 'if' statements below & initialize_module() statements
        to cv_initialize.
    '''
